...
void SettingsDialog::showStabilizerAvMethodOptions(StrokeStabilizer::AveragingMethod method) {
    ...
    bool showOneEuro = method == StrokeStabilizer::AveragingMethod::ONE_EURO;
    gtk_widget_set_visible(builder.get("lbStabilizerMinCutoff"), showOneEuro);
    gtk_widget_set_visible(builder.get("sbStabilizerMinCutoff"), showOneEuro);
    gtk_widget_set_visible(builder.get("lbStabilizerBeta"), showOneEuro);
    gtk_widget_set_visible(builder.get("sbStabilizerBeta"), showOneEuro);

    bool showGoogleInk = method == StrokeStabilizer::AveragingMethod::GOOGLE_INK;
    gtk_widget_set_visible(builder.get("lbStabilizerPredictionTime"), showGoogleInk);
    gtk_widget_set_visible(builder.get("sbStabilizerPredictionTime"), showGoogleInk);

    GtkWidget* preprocessorCombo = builder.get("cbStabilizerPreprocessors");
    gtk_widget_set_sensitive(preprocessorCombo, !showGoogleInk);
    showStabilizerPreprocessorOptions(
            static_cast<StrokeStabilizer::Preprocessor>(gtk_combo_box_get_active(GTK_COMBO_BOX(preprocessorCombo))));
}

void SettingsDialog::showStabilizerPreprocessorOptions(StrokeStabilizer::Preprocessor preprocessor) {
    const bool googleInk = static_cast<StrokeStabilizer::AveragingMethod>(gtk_combo_box_get_active(
                                   GTK_COMBO_BOX(builder.get("cbStabilizerAveragingMethods")))) ==
                           StrokeStabilizer::AveragingMethod::GOOGLE_INK;
    bool showDeadzone = !googleInk && preprocessor == StrokeStabilizer::Preprocessor::DEADZONE;
    ...
    bool showInertia = !googleInk && preprocessor == StrokeStabilizer::Preprocessor::INERTIA;
    ...
    bool sensitive = !googleInk && (showDeadzone || showInertia || averagingOn);
...
void SettingsDialog::load() {
    ...
    GtkWidget* sbStabilizerMinCutoff = builder.get("sbStabilizerMinCutoff");
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(sbStabilizerMinCutoff), settings->getStabilizerMinCutoff());
    GtkWidget* sbStabilizerBeta = builder.get("sbStabilizerBeta");
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(sbStabilizerBeta), settings->getStabilizerBeta());
    GtkWidget* sbStabilizerPredictionTime = builder.get("sbStabilizerPredictionTime");
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(sbStabilizerPredictionTime), settings->getStabilizerPredictionTime());

    GtkComboBox* cbStabilizerAveragingMethods = GTK_COMBO_BOX(builder.get("cbStabilizerAveragingMethods"));
    auto averagingMethod = settings->getStabilizerAveragingMethod();
#ifndef XOURNALPP_ENABLE_INK_STROKE_MODELER
    // The Google backend is an optional build dependency. Do not advertise a
    // mode that this binary would only be able to replace with One Euro.
    GtkTreeIter googleInkRow;
    GtkTreeModel* averagingModel = gtk_combo_box_get_model(cbStabilizerAveragingMethods);
    if (gtk_tree_model_iter_nth_child(averagingModel, &googleInkRow, nullptr,
                                     static_cast<int>(StrokeStabilizer::AveragingMethod::GOOGLE_INK))) {
        gtk_list_store_remove(GTK_LIST_STORE(averagingModel), &googleInkRow);
    }
    if (averagingMethod == StrokeStabilizer::AveragingMethod::GOOGLE_INK) {
        averagingMethod = StrokeStabilizer::AveragingMethod::ONE_EURO;
    }
#endif
    gtk_combo_box_set_active(cbStabilizerAveragingMethods, static_cast<int>(averagingMethod));
    showStabilizerAvMethodOptions(averagingMethod);
...
void SettingsDialog::save() {
    ...
    settings->setStabilizerMinCutoff(gtk_spin_button_get_value(GTK_SPIN_BUTTON(builder.get("sbStabilizerMinCutoff"))));
    settings->setStabilizerBeta(gtk_spin_button_get_value(GTK_SPIN_BUTTON(builder.get("sbStabilizerBeta"))));
    settings->setStabilizerPredictionTime(
            gtk_spin_button_get_value(GTK_SPIN_BUTTON(builder.get("sbStabilizerPredictionTime"))));

...
public:
    ...
    /**
     * Replaces the complete transient prediction tail. The base view ignores
     * this request; predictive views render it without adding it to the mask.
     */
    static constexpr struct PredictionReplacementRequest {
    } PREDICTION_REPLACEMENT_REQUEST = {};
    virtual void on(PredictionReplacementRequest, const std::vector<Point>& prediction);

    static constexpr struct CancellationRequest {
    } CANCELLATION_REQUEST = {};
    virtual void deleteOn(CancellationRequest, const Range& rg);
    ...
    virtual void deleteOn(FinalizationRequest, const Range& rg);

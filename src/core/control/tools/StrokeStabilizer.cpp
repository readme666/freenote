...
namespace {
// Pressure is dimensionless and typically near [0, 1], unlike page
// coordinates. A higher minimum cutoff keeps line-width changes responsive
// while still removing digitizer chatter.
constexpr double PRESSURE_MIN_CUTOFF = 5.0;
constexpr double PRESSURE_BETA = 0.5;
// A slightly faster derivative filter opens the position cutoff within a
// short 120 Hz pen stroke instead of spending most of the stroke ramping up.
constexpr double POSITION_DERIVATIVE_CUTOFF = 2.0;
}  // namespace
...
auto StrokeStabilizer::get(Settings* settings) -> std::unique_ptr<StrokeStabilizer::Base> {
    ...
    // StrokeHandler owns the Google modeler's confirmed/predicted split. Keep
    // One Euro as a fail-open fallback for tools and builds where that pipeline
    // is unavailable.
    if (averagingMethod == AveragingMethod::GOOGLE_INK) {
        return std::make_unique<StrokeStabilizer::OneEuro>(settings->getStabilizerFinalizeStroke(),
                                                           settings->getStabilizerMinCutoff(),
                                                           settings->getStabilizerBeta());
    }
    ...
    if (averagingMethod == AveragingMethod::ONE_EURO) {
        if (preprocessor == Preprocessor::DEADZONE) {
            return std::make_unique<StrokeStabilizer::OneEuroDeadzone>(
                    settings->getStabilizerFinalizeStroke(), settings->getStabilizerMinCutoff(),
                    settings->getStabilizerBeta(), settings->getStabilizerDeadzoneRadius(),
                    settings->getStabilizerCuspDetection());
        }

        if (preprocessor == Preprocessor::INERTIA) {
            return std::make_unique<StrokeStabilizer::OneEuroInertia>(
                    settings->getStabilizerFinalizeStroke(), settings->getStabilizerMinCutoff(),
                    settings->getStabilizerBeta(), settings->getStabilizerDrag(), settings->getStabilizerMass());
        }
...
/**
 * StrokeStabilizer::OneEuro
 */
StrokeStabilizer::OneEuro::OneEuro(bool finalize, double minCutoff, double beta):
        Active(finalize),
        minCutoff(minCutoff),
        beta(beta),
        positionFilter(minCutoff, beta, POSITION_DERIVATIVE_CUTOFF),
        pressureFilter(PRESSURE_MIN_CUTOFF, PRESSURE_BETA) {}

void StrokeStabilizer::OneEuro::resetFilters(const Event& ev, guint32 timestamp) {
    this->positionFilter.reset({ev.x, ev.y}, timestamp);
    this->pressureInitialized = ev.pressure != Point::NO_PRESSURE && std::isfinite(ev.pressure);
    if (this->pressureInitialized) {
        this->pressureFilter.reset(ev.pressure, timestamp);
        this->lastFilteredPressure = ev.pressure;
    }
}

void StrokeStabilizer::OneEuro::recordFirstEvent(const PositionInputData& pos) {
    this->lastEvent = Event(pos);
    this->resetFilters(this->lastEvent, pos.timestamp);
}

void StrokeStabilizer::OneEuro::averageAndPaint(const Event& ev, guint32 timestamp) {
    this->lastEvent = ev;

    const auto position = this->positionFilter.filter({ev.x, ev.y}, timestamp);
    Event filtered(position.x, position.y, ev.pressure);
    if (ev.pressure != Point::NO_PRESSURE && std::isfinite(ev.pressure)) {
        if (!this->pressureInitialized) {
            this->pressureFilter.reset(ev.pressure, timestamp);
            this->pressureInitialized = true;
            this->lastFilteredPressure = ev.pressure;
        } else {
            filtered.pressure = this->pressureFilter.filter(ev.pressure, timestamp);
            this->lastFilteredPressure = filtered.pressure;
        }
    } else {
        filtered.pressure = this->pressureInitialized ? this->lastFilteredPressure : Point::NO_PRESSURE;
    }

    // The endpoint finisher uses lastEvent, so retain the raw endpoint
    // coordinates while keeping its width continuous with the filtered stroke.
    this->lastEvent.pressure = filtered.pressure;

    setLastPaintedEvent(filtered);
    drawEvent(filtered);
}

auto StrokeStabilizer::OneEuro::getLastEvent() -> Event { return this->lastEvent; }

void StrokeStabilizer::OneEuro::resetBuffer(Event& ev, guint32 timestamp) {
    this->lastEvent = ev;
    this->resetFilters(ev, timestamp);
}

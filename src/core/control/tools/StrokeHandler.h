...
protected:
    ...
    /// Finalizes the stroke using the release event as the final input.
    void finalizeStroke(const PositionInputData& pos);

    void appendModeledPoints(const std::vector<StrokePrediction::ModeledPoint>& points);
    void replacePrediction(const std::vector<StrokePrediction::ModeledPoint>& prediction);
    void clearPrediction();
    [[nodiscard]] auto makePredictionInput(const PositionInputData& pos) const -> StrokePrediction::InputSample;
    [[nodiscard]] auto makeRawPoint(const PositionInputData& pos, double zoom) const -> Point;
    [[nodiscard]] auto resolvePressure(double pressure) const -> double;
...
private:
    ...
    std::unique_ptr<StrokePrediction::InkStrokeModelerAdapter> inkModeler;
    bool useGoogleInk{false};
    bool googleInkRawFallback{false};

...
#include "control/tools/OneEuroFilter.h"         // for OneEuroFilter
...
/**
 * @brief Time-aware adaptive low-pass stabilizer based on the 1 Euro filter.
 *
 * Slow input is filtered strongly to remove digitizer jitter. The cutoff
 * rises with movement speed so fast handwriting stays close to the pen tip.
 * Pressure is filtered independently with parameters tuned for its typically
 * normalized, dimensionless range.
 */
class OneEuro: virtual public Active {
public:
    OneEuro(bool finalize, double minCutoff, double beta);
    ~OneEuro() override = default;

    [[maybe_unused]] auto getInfo() -> std::string override {
        return "1 Euro stabilizer with minCutoff = " + std::to_string(minCutoff) + ", beta = " + std::to_string(beta);
    }
...
private:
    void resetFilters(const Event& ev, guint32 timestamp);

    const double minCutoff;
    const double beta;
    OneEuroFilter2D positionFilter;
    OneEuroFilter pressureFilter;
    Event lastEvent;
    bool pressureInitialized{false};
    double lastFilteredPressure{};
};
...
class OneEuroDeadzone: public OneEuro, public Deadzone {
public:
    OneEuroDeadzone(bool finalize, double minCutoff, double beta, double dzRadius, bool cuspDetection):
            Active(finalize), OneEuro(finalize, minCutoff, beta), Deadzone(finalize, dzRadius, cuspDetection) {}
    ~OneEuroDeadzone() override = default;
...
private:
    inline void recordFirstEvent(const PositionInputData& pos) override {
        OneEuro::recordFirstEvent(pos);
        Deadzone::recordFirstEvent(pos);
    }
    inline Event getLastEvent() override {
        Event endpoint = Deadzone::getLastEvent();
        endpoint.pressure = OneEuro::getLastEvent().pressure;
        return endpoint;
    }
};

class OneEuroInertia: public OneEuro, public Inertia {
public:
    OneEuroInertia(bool finalize, double minCutoff, double beta, double drag, double mass):
            Active(finalize), OneEuro(finalize, minCutoff, beta), Inertia(finalize, drag, mass) {}
    ~OneEuroInertia() override = default;
...
private:
    ...
    inline Event getLastEvent() override {
        Event endpoint = Inertia::getLastEvent();
        endpoint.pressure = OneEuro::getLastEvent().pressure;
        return endpoint;
    }
};

...
namespace {
double finiteClamp(double value, double lower, double upper, double fallback) {
...
void Settings::loadDefault() {
    ...
    this->stabilizerAveragingMethod = StrokeStabilizer::AveragingMethod::GOOGLE_INK;
#else
    this->stabilizerAveragingMethod = StrokeStabilizer::AveragingMethod::ONE_EURO;
#endif
...
void Settings::parseItem(xmlDocPtr doc, xmlNodePtr cur) {
    ...
    if (xmlStrcmp(name, reinterpret_cast<const xmlChar*>("pressureSensitivity")) == 0) {
        ...
        const auto method = static_cast<StrokeStabilizer::AveragingMethod>(
                g_ascii_strtoll(reinterpret_cast<const char*>(value), nullptr, 10));
        this->stabilizerAveragingMethod =
                StrokeStabilizer::isValid(method) ? method : StrokeStabilizer::AveragingMethod::NONE;
    } else if (xmlStrcmp(name, reinterpret_cast<const xmlChar*>("stabilizerPreprocessor")) == 0) {
        const auto preprocessor = static_cast<StrokeStabilizer::Preprocessor>(
                g_ascii_strtoll(reinterpret_cast<const char*>(value), nullptr, 10));
    ...
    } else if (xmlStrcmp(name, reinterpret_cast<const xmlChar*>("stabilizerMinCutoff")) == 0) {
        this->stabilizerMinCutoff = finiteClamp(tempg_ascii_strtod(reinterpret_cast<const char*>(value), nullptr), 0.1,
                                                10.0, this->stabilizerMinCutoff);
    } else if (xmlStrcmp(name, reinterpret_cast<const xmlChar*>("stabilizerBeta")) == 0) {
        this->stabilizerBeta = finiteClamp(tempg_ascii_strtod(reinterpret_cast<const char*>(value), nullptr), 0.0, 0.1,
                                           this->stabilizerBeta);
    } else if (xmlStrcmp(name, reinterpret_cast<const xmlChar*>("stabilizerPredictionTime")) == 0) {
        this->stabilizerPredictionTime = finiteClamp(tempg_ascii_strtod(reinterpret_cast<const char*>(value), nullptr),
                                                     4.0, 20.0, this->stabilizerPredictionTime);
...
void Settings::save() {
    ...
    xmlNodePtr com = xmlNewComment(
            reinterpret_cast<const xmlChar*>("The freenote settings file. Do not edit this file! "
    ...
    SAVE_DOUBLE_PROP(stabilizerMinCutoff);
    SAVE_DOUBLE_PROP(stabilizerBeta);
    SAVE_DOUBLE_PROP(stabilizerPredictionTime);
...
auto Settings::getStabilizerMinCutoff() const -> double { return stabilizerMinCutoff; }
auto Settings::getStabilizerBeta() const -> double { return stabilizerBeta; }
auto Settings::getStabilizerPredictionTime() const -> double { return stabilizerPredictionTime; }
...
void Settings::setStabilizerMinCutoff(double minCutoff) {
    const double clamped = finiteClamp(minCutoff, 0.1, 10.0, stabilizerMinCutoff);
    if (stabilizerMinCutoff == clamped) {
        return;
    }
    stabilizerMinCutoff = clamped;
    save();
}
void Settings::setStabilizerBeta(double beta) {
    const double clamped = finiteClamp(beta, 0.0, 0.1, stabilizerBeta);
    if (stabilizerBeta == clamped) {
        return;
    }
    stabilizerBeta = clamped;
    save();
}
void Settings::setStabilizerPredictionTime(double predictionTime) {
    const double clamped = finiteClamp(predictionTime, 4.0, 20.0, stabilizerPredictionTime);
    if (stabilizerPredictionTime == clamped) {
        return;
    }
    stabilizerPredictionTime = clamped;
    save();
}

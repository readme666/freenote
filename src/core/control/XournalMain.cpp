    ...
    if (!fs::exists(newConfigPath)) {
        ...
        for (auto const& oldPath: oldPaths) {
            ...
            try {
                ...
                constexpr auto msg = "Due to a recent update, freenote has changed where its configuration files are "
                ...
                constexpr auto msg =
                        "Due to a recent update, freenote has changed where its configuration files are "
...
void checkForEmergencySave(Control* control) {
    ...
    const std::string msg = _("freenote crashed last time. Would you like to restore the last edited file?");
...
auto findResourcePath(const fs::path& searchFile) -> fs::path {
    auto search_for = [&searchFile](fs::path start) -> std::optional<fs::path> {
        constexpr auto* postfix = "share/freenote";
        /// 1. relative install
        /// 2. windows install
        /// 3. build dir
...
void on_open_files(GApplication* application, gpointer f, gint numFiles, gchar* hint, XMPtr app_data) {
    ...
    auto* files = (GFile**)f;
    if (numFiles != 1) {
        const std::string msg = _("Sorry, freenote can only open one file at once.\n"
                                  "Others are ignored.");
    ...
    try {
        ...
        const std::string msg = FS(_F("Filesystem error: {1}\n"
                                      "Sorry, freenote cannot open the file: {2}\n"
...
void on_startup(GApplication* application, XMPtr app_data) {
    ...
    initResourcePath(app_data->gladePath.get(), "ui/freenote.css", false);
...
auto XournalMain::run(int argc, char** argv) -> int {
    ...
    GtkApplication* app = gtk_application_new("io.github.readme666.freenote", APP_FLAGS);
    g_object_set(G_OBJECT(app), "register-session", true, nullptr);  // Needed for opening files on MacOS from Finder
    g_set_prgname("io.github.readme666.freenote");
    ...
    std::array options = {GOptionEntry{"page", 'n', 0, G_OPTION_ARG_INT, &app_data.openAtPageNumber,
                          ...
                          GOptionEntry{"version", 0, 0, G_OPTION_ARG_NONE, &app_data.showVersion,
                                       _("Get version of freenote"), nullptr},

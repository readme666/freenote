...
* Added low-latency adaptive input stabilization and pressure smoothing for handwriting
* Improved fast and diagonal handwriting response with direction-independent velocity filtering
* Added optional Google Ink Stroke Modeler prediction with a non-persistent wet-ink preview
* Fixed stroke prediction being permanently disabled mid-stroke by an out-of-order input timestamp, by a pause longer than half a second, or by a mid-stroke zoom change; these are now treated as sub-stroke boundaries and the model is re-seeded instead
* Fixed stroke prediction never becoming visible on devices reporting faster than 1 kHz, and the model time drift such devices caused
* Fixed `-DENABLE_INK_STROKE_MODELER=ON` failing to configure because the vendored modeler resolved GoogleTest unconditionally, even though its own tests are disabled and the targets are never referenced
* Reduced the CPU cost of the prediction pass by recomputing it at the display cadence instead of on every input event
